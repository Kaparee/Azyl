...
@section('content')
    ...
    <div class="relative bg-gray-900 min-h-[600px] flex flex-col justify-center items-center text-center">
        ...
        <div class="relative z-10 max-w-4xl px-4 mt-8 flex flex-col items-center">
            ...
            <div class="mt-8 flex flex-col sm:flex-row gap-4">
                ...
                <a href="#" class="bg-transparent text-white border border-white/40 px-8 py-3.5 rounded-full font-semibold hover:bg-white/10 transition flex items-center justify-center gap-2">
                    Wspieraj nas
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div class="flex justify-between items-end mb-8">
            ...
            <a href="#" class="hidden sm:flex text-azyl-orange font-medium hover:underline items-center text-sm">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition group flex flex-col">
                <div class="relative h-48 bg-gray-200 shrink-0">
                    <img src="{{ asset('images/hero_shelter.png') }}" alt="{{ $animal->name }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500">
                    @if($index == 0)
                ...
                <div class="p-5 flex flex-col flex-grow">
                    <div class="flex justify-between items-start mb-2">
                        <span class="text-xs font-bold text-gray-500 uppercase">{{ $animal->breed->species->name ?? 'Zwierzę' }}</span>
                        <span class="text-[10px] font-bold bg-green-100 text-green-700 px-2 py-1 rounded-md">Dostępny do adopcji</span>
                    </div>
                    <h3 class="text-xl font-bold text-gray-900 mb-1">{{ $animal->name }}</h3>
                    <p class="text-sm text-gray-500 mb-4">{{ $animal->breed->name ?? 'Mieszaniec' }}</p>
                    
                    <div class="flex gap-x-4 text-xs text-gray-600 mb-4">
                        <div class="flex items-center"><svg class="w-4 h-4 mr-1 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> {{ $animal->age_months }} mies.</div>
                        <div class="flex items-center">
                            @if($animal->genders == 1) Samiec @else Samica @endif
                        </div>
                    </div>
                    
                    <p class="text-sm text-gray-600 line-clamp-2 mb-5 flex-grow">
                        {{ $animal->description ?? 'Wspaniałe zwierzę szukające kochającego domu. Jest bardzo przyjazne i radosne.' }}
                    </p>
                    
                    <a href="#" class="block w-full text-center border border-gray-300 text-gray-700 font-medium py-2 rounded-lg hover:border-azyl-orange hover:text-azyl-orange transition">Zobacz</a>
                </div>
            </div>
            @endforeach
            @if($recentAnimals->count() == 0)
                <p class="col-span-4 text-gray-500 text-center py-8">Brak zwierząt w katalogu.</p>
            @endif
        </div>
        <div class="mt-6 sm:hidden text-center">
            <a href="#" class="text-azyl-orange font-medium hover:underline inline-flex items-center text-sm">
                Zobacz wszystkie <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
            </a>
        </div>
    </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
                <div class="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col h-full hover:shadow-md transition">
                    ...
                    <a href="#" class="text-azyl-orange font-medium hover:underline flex items-center text-sm mt-auto">Rozpocznij <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg></a>
                </div>
                <div class="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col h-full hover:shadow-md transition">
                    <div class="text-4xl mb-4">💖</div>
                    ...
                    <a href="#" class="text-azyl-orange font-medium hover:underline flex items-center text-sm mt-auto">Wybierz podopiecznego <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg></a>
                </div>
                <div class="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col h-full hover:shadow-md transition">
                    <div class="text-4xl mb-4">🤝</div>
                    ...
                    <a href="#" class="text-azyl-orange font-medium hover:underline flex items-center text-sm mt-auto">Dołącz do nas <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg></a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-azyl-orange py-12">
        <div class="max-w-4xl mx-auto px-4 text-center flex flex-col items-center">
            ...
            <a href="#" class="bg-white text-azyl-orange px-8 py-3.5 rounded-full font-bold shadow-md hover:bg-gray-50 transition flex items-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>
