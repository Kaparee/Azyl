    ...
    <div class="max-w-xl mx-auto">
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm">
            <div class="border-b border-slate-200 px-6 py-4">
                <h1 class="text-xl font-bold">Edytuj rasę</h1>
            </div>

            <form method="POST" action="{{ route('admin.breeds.update', $breed) }}" class="p-6">
                @csrf
                @method('PUT')

                <div class="mb-4">
                    <label class="block text-sm mb-1">Nazwa rasy *</label>
                    <input name="name" value="{{ old('name', $breed->name) }}" class="w-full rounded-xl border-slate-200" required>
                ...
                <div class="mb-4">
                    ...
                    <select name="species_id" class="w-full rounded-xl border-slate-200" required>
                        ...
                        @foreach ($species as $item)
                            <option value="{{ $item->id }}" @selected(old('species_id', $breed->species_id) == $item->id)>
                                {{ $item->name }}
                            </option>
                        @endforeach
                    </select>
