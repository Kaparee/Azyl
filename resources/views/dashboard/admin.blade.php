        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col justify-between">
                <div class="flex justify-between items-start">
                    ...
                    <span class="text-sm font-bold flex items-center gap-1 {{ $animalsDiff > 0 ? 'text-green-500' : ($animalsDiff < 0 ? 'text-red-500' : 'text-gray-500') }}">
                        @if($animalsDiff > 0)
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col justify-between">
                <div class="flex justify-between items-start">
                    ...
                    <span class="text-sm font-bold flex items-center gap-1 {{ $applicationsDiff > 0 ? 'text-green-500' : ($applicationsDiff < 0 ? 'text-red-500' : 'text-gray-500') }}">
                        @if($applicationsDiff > 0)
                            <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 10l7-7m0 0l7 7m-7-7v18"/></svg>
                            +{{ $applicationsDiff }}%
                        @elseif($applicationsDiff < 0)
                            <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 14l-7 7m0 0l-7-7m7 7V3"/></svg>
                            {{ $applicationsDiff }}%
                        @else
                            0%
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col justify-between">
                <div class="flex justify-between items-start">
                    ...
                    <span class="text-sm font-bold flex items-center gap-1 {{ $donationsDiff > 0 ? 'text-green-500' : ($donationsDiff < 0 ? 'text-red-500' : 'text-gray-500') }}">
                        @if($donationsDiff > 0)
                            <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 10l7-7m0 0l7 7m-7-7v18"/></svg>
                            +{{ $donationsDiff }}%
                        @elseif($donationsDiff < 0)
                            <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 14l-7 7m0 0l-7-7m7 7V3"/></svg>
                            {{ $donationsDiff }}%
                        @else
                            0%
