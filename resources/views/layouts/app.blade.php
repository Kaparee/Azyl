...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body x-data="{ sidebarOpen: false }" class="font-sans antialiased bg-gray-50 text-gray-900 overflow-x-hidden">
        ...
        <!-- Main content column -->
        <div class="min-h-screen w-full max-lg:pl-0 lg:pl-64 flex flex-col min-w-0">
            @include('layouts.topbar')

            <main class="flex-1 p-4 sm:p-6 lg:p-8 min-w-0 relative">
                <!-- Global Toasts -->
                @if(session('success') || session('error'))
                    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)"
                         class="absolute top-4 right-4 sm:right-8 z-50 transition-all duration-300 transform"
                        ...
                        @if(session('error'))
                            <div class="bg-red-50 text-red-800 border border-red-200 rounded-xl p-4 shadow-lg flex items-center gap-3">
                                <svg class="w-6 h-6 text-red-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
